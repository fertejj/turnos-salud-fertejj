...
export default function EditMedicalHistoryPage() {
  ...
  useEffect(() => {
    const loadEntry = async () => {
      ...
      if (entry) {
        setValue("date", format(entry.date, "yyyy-MM-dd")) // ✅ input[type="date"] compatible
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const parsedData = {
        ...data,
        date: new Date(data.date), // ← convertir string a Date antes de actualizar
      }

      await updateEntry(entryId!, parsedData)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Campos opcionales */}
        {["description", "note", "signs", "diagnosis", "treatment"].map((name) => (
          <div key={name}>
            <label className="block text-sm font-medium text-[var(--color-text)] mb-1">
